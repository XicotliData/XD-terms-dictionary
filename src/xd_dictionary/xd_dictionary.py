...
def get_values_of(of_id: int, values_df: pd.DataFrame) -> list:
    ...
    return values_df.loc[
        values_df["observationFieldID"] == of_id, ["values", "url"]
    ].to_dict(orient="records")


def process_xd_dict(xd_dict_path: BinaryIO) -> pd.DataFrame:
    """proccess a xd dictionary file"""
    values_obs_fields_data = (
        pd.read_excel(xd_dict_path, sheet_name=1)
        .rename(columns={"Unnamed: 4": "url"})
        .loc[:, ["observationFieldID", "values", "url"]]
    )
    obs_fields_data = (
        pd.read_excel(xd_dict_path, sheet_name=0)
        .rename(
            columns={
        ...
        .loc[:, ["observationFieldID", "of_name", "of_id", "url", "description"]]
        .assign(
            of_values=lambda x: x["observationFieldID"].apply(
                get_values_of, args=(values_obs_fields_data,)
            )
        )
    )

    obs_fields_data.loc[
        obs_fields_data["of_values"].apply(len) == 0, "of_values"
    ] = None
...
def main(
    verbose: int, task: str, xd_file: BinaryIO, build_dir: Union[Path, None] = None
) -> None:
    ...
    env = Environment(loader=PackageLoader("xd_dictionary"))
    ...
    with fn.open("w") as f:
        f.write(
            terms_tmpl.render(terms=xd_data.to_dict(orient="records"), output_dir=True)
        )
